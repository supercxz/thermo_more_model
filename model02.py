...
for i in range(1, 301):
    ...
    if best_mse > mse:
        ...
        best_idx = i
        best_r2 = r2
print(f'在随机森林调优 n_neighbors 过程中，最好的效果：mse:{best_mse:.5f}, r2:{best_r2:.5f}, n_neighbors:{best_idx}')
...
for i in range(1, 301):
    print(f'第{i+1}个')
    clf = KNeighborsRegressor(n_neighbors=4, leaf_size=i)
    # 在训练集上拟合模型
    clf.fit(X_train,y_train)
    # 对测试集进行预测
    label_pred = clf.predict(X_test)
...
for i in range(1, 11):
    print(f'第{i+1}个')
    clf = KNeighborsRegressor(n_neighbors=4, leaf_size=1, p=i)
    # 在训练集上拟合模型
    clf.fit(X_train,y_train)
    # 对测试集进行预测
    label_pred = clf.predict(X_test)
...
#%%
# 创建随机森林分类器对象
clf = KNeighborsRegressor(n_neighbors=4, leaf_size=1, p=2)
# 在训练集上拟合模型
clf.fit(X_train, y_train)
